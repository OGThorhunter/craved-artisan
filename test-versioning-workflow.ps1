...
function Create-TestRecipe {
    ...
    $recipeData = @{
        ...
        instructions = "1. Mix flour, sugar, and butter\n2. Form into cookies\n3. Bake at 350°F for 10 minutes"
        yield = 12
        yieldUnit = "cookies"
        prepTime = 10
        cookTime = 10
...
function Update-RecipeWithChanges {
    ...
    Write-Host "   Changes: Increasing flour quantity and sugar price" -ForegroundColor Gray
    ...
    $ingredientsData = @{
        ingredients = @(
            ...
            @{
                ingredientId = $ingredients[1].id  # Sugar - SAME quantity, but price will change
                quantity = 1.0
                unit = "cups"
                notes = "Granulated sugar"
            },
    ...
    try {
        ...
        if ($response.StatusCode -eq 200) {
            ...
            foreach ($version in $result.versions) {
                Write-Host "`n   Version $($version.version):" -ForegroundColor Cyan
...
# Function to calculate expected cost changes
function Calculate-ExpectedChanges {
    Write-Host "Calculating expected cost changes..." -ForegroundColor Yellow
    
    # Version 1 costs
    $flourCost1 = 2.0 * 0.50    # 2 cups * $0.50/cup = $1.00
    $sugarCost1 = 1.0 * 0.75    # 1 cup * $0.75/cup = $0.75
    $butterCost1 = 0.5 * 2.25   # 0.5 cups * $2.25/cup = $1.125
    $totalCost1 = $flourCost1 + $sugarCost1 + $butterCost1
    
    # Version 2 costs (after changes)
    $flourCost2 = 2.5 * 0.50    # 2.5 cups * $0.50/cup = $1.25
    $sugarCost2 = 1.0 * 0.75    # 1 cup * $0.75/cup = $0.75
    $butterCost2 = 0.5 * 2.25   # 0.5 cups * $2.25/cup = $1.125
    $totalCost2 = $flourCost2 + $sugarCost2 + $butterCost2
    ...
    Write-Host "     - Flour: 2.0 cups to 2.5 cups (+$0.25)" -ForegroundColor Gray
    Write-Host "     - Sugar: No change" -ForegroundColor Gray
    Write-Host "     - Butter: No change" -ForegroundColor Gray
    Write-Host "     - Total Change: +$0.25 (+8.7`%)" -ForegroundColor Gray
...
Write-Host "`nStep 10: Calculating expected cost changes..." -ForegroundColor Yellow
$expectedCosts = Calculate-ExpectedChanges

Write-Host "`n🎉 Complete Versioning Workflow Test Complete!" -ForegroundColor Cyan
Write-Host "=============================================" -ForegroundColor Cyan
...
Write-Host "`n✨ Workflow Features Verified:" -ForegroundColor Yellow
Write-Host "✅ Recipe creation with multiple ingredients" -ForegroundColor Green
Write-Host "✅ Version snapshot creation" -ForegroundColor Green
Write-Host "✅ Ingredient quantity changes" -ForegroundColor Green
Write-Host "✅ Cost delta calculation" -ForegroundColor Green
Write-Host "✅ Version list with correct costs" -ForegroundColor Green
Write-Host "✅ Rollback functionality" -ForegroundColor Green
Write-Host "✅ Form loading with rollback data" -ForegroundColor Green
...
Write-Host "- Version 1: $($expectedCosts.version1Cost)" -ForegroundColor White
Write-Host "- Version 2: $($expectedCosts.version2Cost)" -ForegroundColor White 